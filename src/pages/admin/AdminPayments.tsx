...
const AdminPayments = () => {
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      if (payment.plan_requested === 'Pro') limit = 2147483647; // Unlimited for Postgres int4
      ...
      // 3. Mark the EXACT payment record as approved
      ...
      toast.success(`Plan activated: ${payment.plan_requested} for ${payment.clinics.name}`);
      await fetchPayments(); // Refresh list to disable buttons
    } catch (err: any) {
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      toast.success("Payment rejected for " + payment.clinics?.name);
      await fetchPayments();
    } catch (err: any) {
      ...
      <div className="max-w-6xl mx-auto space-y-10">
        <div className="flex justify-between items-end">
          ...
          <Button onClick={fetchPayments} variant="outline" className="rounded-xl h-12 px-6 font-bold" disabled={loading}>
            {loading ? <Loader2 className="animate-spin w-4 h-4 mr-2" /> : "Sync Requests"}
          </Button>
        </div>
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-slate-50/50 border-none">
                ...
                <TableHead className="py-6 px-4 font-black text-[10px] uppercase tracking-widest text-slate-400">Transaction Details</TableHead>
            ...
            <TableBody>
              {loading && payments.length === 0 ? (
                <TableRow><TableCell colSpan={5} className="h-48 text-center"><Loader2 className="animate-spin mx-auto text-indigo-600" /></TableCell></TableRow>
              ) : payments.length === 0 ? (
                <TableRow><TableCell colSpan={5} className="h-48 text-center text-slate-400 font-bold uppercase text-xs tracking-widest">No requests found</TableCell></TableRow>
              ) : (
                payments.map((p) => (
                  <TableRow key={p.id} className="hover:bg-slate-50/50 transition-all border-b border-slate-50">
                    <TableCell className="px-8 py-6">
                      ...
                      <p className="text-[10px] text-slate-400 font-medium truncate max-w-[120px]">UID: {p.clinic_id}</p>
                    </TableCell>
                    <TableCell className="px-4 py-6">
                      <Badge className={cn("px-3 py-1 rounded-lg", p.plan_requested === 'Pro' ? 'bg-indigo-600' : 'bg-slate-900')}>
                        {p.plan_requested}
                      </Badge>
                    </TableCell>
                    <TableCell className="px-4 py-6">
                      ...
                      <code className="text-[10px] bg-slate-100 px-2 py-1 rounded text-indigo-600 font-black tracking-tight">{p.transaction_id}</code>
                    </TableCell>
                    <TableCell className="px-4 py-6">
                      <Badge variant="outline" className={cn(
                        "rounded-full px-3 py-1 font-black uppercase text-[10px] tracking-widest shadow-none border-2",
                        p.status === 'pending' ? 'border-amber-200 text-amber-500 bg-amber-50/30' : 
                        p.status === 'approved' ? 'border-emerald-200 text-emerald-500 bg-emerald-50' : 'border-rose-200 text-rose-500 bg-rose-50'
                      )}>
                        {p.status}
                      </Badge>
                    </TableCell>
                    <TableCell className="px-8 py-6 text-right">
                      {p.status === 'pending' ? (
                        <div className="flex justify-end gap-2">
                          ...
                          <Button 
                            className="rounded-xl bg-emerald-600 hover:bg-emerald-700 h-10 px-5 font-bold shadow-md"
                        ...
                        <div className="flex justify-end pr-2">
                           <div className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-[0.15em] text-slate-300">
      ...
      <AlertDialog open={!!confirmApprove} onOpenChange={() => setConfirmApprove(null)}>
        <AlertDialogContent className="rounded-[2.5rem] border-none shadow-2xl p-8">
          <AlertDialogHeader>
            ...
            <AlertDialogTitle className="text-2xl font-black tracking-tight text-slate-900">Activate Plan?</AlertDialogTitle>
            <AlertDialogDescription className="font-medium text-slate-500">
              Activating <span className="text-indigo-600 font-bold">{confirmApprove?.plan_requested}</span> for <span className="text-slate-900 font-bold">{confirmApprove?.clinics?.name}</span>.
              This will update their dashboard instantly.
            </AlertDialogDescription>
          </AlertDialogHeader>
      ...
      <AlertDialog open={!!confirmReject} onOpenChange={() => setConfirmReject(null)}>
        <AlertDialogContent className="rounded-[2.5rem] border-none shadow-2xl p-8">
          <AlertDialogHeader>
            ...
            <AlertDialogTitle className="text-2xl font-black tracking-tight text-slate-900">Reject Request?</AlertDialogTitle>
            <AlertDialogDescription className="font-medium text-slate-500">
              Rejecting payment from <span className="text-slate-900 font-bold">{confirmReject?.clinics?.name}</span>. They will be notified to retry.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className="mt-8 gap-3">
            <AlertDialogCancel className="rounded-xl h-12 border-slate-100 font-bold">Cancel</AlertDialogCancel>
