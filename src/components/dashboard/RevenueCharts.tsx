...
const RevenueCharts = ({ data, fees }: RevenueChartsProps) => {
  ...
  const getRecordRevenue = (p: any) => {
    ...
    // If manual fee is recorded (> 0), use it. Otherwise, fallback to clinic standards for historical data.
    if (manualFee > 0) return manualFee;
    return p.visit_type === 'New' ? Number(fees.new) : Number(fees.followUp);
  };

  const totalRevenue = data.reduce((acc, curr) => acc + getRecordRevenue(curr), 0);

  // 1. Revenue Over Time (Using Smart Calculation)
  ...
  data.forEach(p => {
    ...
    if (p.visit_type === 'New') typeRevenue['New Patients'] += rev;
    else typeRevenue['Follow-up'] += rev;
  });
    ...
    <div className="space-y-8">
      <Card className="p-8 border-none shadow-2xl shadow-emerald-100/20 dark:shadow-none rounded-[2.5rem] bg-white dark:bg-slate-900 flex flex-col md:flex-row items-center justify-between gap-6 group transition-all duration-300">
        <div className="flex items-center gap-6">
          ...
          <div>
            <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1">Total Net Revenue</p>
        ...
        <div className="hidden md:block text-right">
          <p className="text-xs font-bold text-slate-400 dark:text-slate-500">Calculated from {data.length} records</p>
          <div className="mt-2 flex gap-4 text-[10px] font-black uppercase tracking-widest">
            <span className="text-indigo-600">Status: Analytics Synchronized</span>
          </div>
        </div>
      </Card>
