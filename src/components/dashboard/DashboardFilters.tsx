...
const DashboardFilters = ({ filters, onFilterChange, onReset }: DashboardFiltersProps) => {
  return (
    <Card className="bg-white p-5 md:p-8 rounded-[2.5rem] shadow-sm border-none mb-8 animate-in fade-in slide-in-from-top-4 duration-500">
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <h3 className="text-[10px] md:text-sm font-black text-slate-400 uppercase tracking-widest">Active Filters</h3>
          <Button 
            variant="ghost" 
            onClick={onReset}
            className="h-8 md:h-10 px-3 md:px-4 rounded-xl hover:bg-rose-50 text-rose-500 hover:text-rose-600 font-bold text-[10px] md:text-xs transition-colors"
          >
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="relative w-full">
            ...
            <Input 
              ...
              className="pl-12 h-14 rounded-2xl bg-slate-50 border-none focus-visible:ring-indigo-500/20 text-base w-full"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:flex lg:flex-row items-center gap-3">
            ...
            <div className="relative w-full lg:w-[180px]">
              ...
              <Input 
                type="date" 
                className="pl-12 w-full h-14 rounded-2xl bg-slate-50 border-none focus-visible:ring-indigo-500/20"
                value={filters.date}
            ...
            <Select value={filters.gender} onValueChange={(v) => onFilterChange('gender', v)}>
              <SelectTrigger className="w-full lg:w-[150px] h-14 rounded-2xl bg-slate-50 border-none text-slate-600 font-medium">
            ...
            <Select value={filters.visitType} onValueChange={(v) => onFilterChange('visitType', v)}>
              <SelectTrigger className="w-full lg:w-[160px] h-14 rounded-2xl bg-slate-50 border-none text-slate-600 font-medium">
