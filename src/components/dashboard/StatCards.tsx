      ...
      {stats.map((stat) => (
        <Card key={stat.title} className="p-8 border-none shadow-2xl shadow-indigo-100/20 dark:shadow-none rounded-[2.5rem] bg-white dark:bg-slate-900 flex flex-col justify-between hover:shadow-indigo-200/30 dark:hover:bg-slate-800/50 transition-all duration-300 group">
          <div className="flex justify-between items-start mb-6">
            ...
            <ArrowUpRight className="w-5 h-5 text-slate-200 dark:text-slate-700" />
          </div>
          <div>
            ...
            <div className="flex items-baseline gap-3">
              ...
              <div className="flex items-center gap-2 px-4 py-1.5 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-800">
                <CalendarDays className="w-4 h-4 text-slate-400 dark:text-slate-500" />
                <span className="text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-tight">Today</span>
